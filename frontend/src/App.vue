  ...
  <a-layout class="layout">
    <a-layout-header>
      <a-menu theme="dark" mode="horizontal" v-model:selectedKeys="selectedKeys" :style="{ lineHeight: '64px' }">
        <a-menu-item key="1"><router-link to="/">{{title}}</router-link></a-menu-item>
        <template v-if="store.state['user'].isLoggedIn">
          ...
          <a-menu-item key="5"><router-link to="/dashboard/my-account">My account</router-link></a-menu-item>
        </template>
        <template v-else>
